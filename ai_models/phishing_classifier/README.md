...
```python
import torch
from aether_guard.ai_models.phishing_classifier import PhishingClassifierDetector

# Model automatically uses ROCm if available
detector = PhishingClassifierDetector()
...
@register_detector
class PhishingClassifierDetector(Detector):
    name = "phishing_classifier_v1"
    
    def __init__(self):
        # Load model with ROCm device
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = load_classifier_model(device=self.device)
